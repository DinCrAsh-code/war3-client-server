//============================================================================
//  0x6F3BD4D0 - ResolvePlayerHandle: turn a JASS `player` handle into the object
//  behind it, or null.
//
...
//  guess, because both of its constructor calls are 0x6F388950, the '+ply'
//  constructor (checkedplayerslot.cpp).
//
...
SJassPlayer* __fastcall ResolvePlayerHandle(int handle)
{
    ...
    SCheckedPlayerSlot checked(candidate);
    ...
    if (validated)
        return (SJassPlayer*)checked.m_value;

    SCheckedPlayerSlot fallback(0);
    return (SJassPlayer*)fallback.m_value;
}
