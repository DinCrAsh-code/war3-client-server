//============================================================================
//  0x6F3BDEF0 - ResolveTriggerHandle: turn a JASS `trigger` handle into the object
//  behind it, or null.
//
...
//  guess, because both of its constructor calls are 0x6F3B91E0, the '+trg'
//  constructor (checkedtriggerslot.cpp).
//
...
SJassTrigger* __fastcall ResolveTriggerHandle(int handle)
{
    ...
    SCheckedTriggerSlot checked(candidate);
    ...
    if (validated)
        return (SJassTrigger*)checked.m_value;

    SCheckedTriggerSlot fallback(0);
    return (SJassTrigger*)fallback.m_value;
}
