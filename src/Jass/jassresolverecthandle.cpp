//============================================================================
//  0x6F3BDB90 - ResolveRectHandle: turn a JASS `rect` handle into the object
//  behind it, or null.
//
...
//  guess, because both of its constructor calls are 0x6F3B9180, the '+rct'
//  constructor (checkedrectslot.cpp).
//
...
SJassRect* __fastcall ResolveRectHandle(int handle)
{
    ...
    SCheckedRectSlot checked(candidate);
    ...
    if (validated)
        return (SJassRect*)checked.m_value;

    SCheckedRectSlot fallback(0);
    return (SJassRect*)fallback.m_value;
}
