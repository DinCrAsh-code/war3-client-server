//============================================================================
//  0x6F3BD710 - ResolveTimerHandle: turn a JASS `timer` handle into the object
//  behind it, or null.
//
...
//  guess, because both of its constructor calls are 0x6F34BEA0, the '+tmr'
//  constructor (checkedtimerslot.cpp).
//
...
SJassTimer* __fastcall ResolveTimerHandle(int handle)
{
    ...
    SCheckedTimerSlot checked(candidate);
    ...
    if (validated)
        return (SJassTimer*)checked.m_value;

    SCheckedTimerSlot fallback(0);
    return (SJassTimer*)fallback.m_value;
}
